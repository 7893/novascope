...
* **服务账号**:
    Cloud Functions 与 Cloud Scheduler 统一使用项目的 Compute Engine 默认服务账号 `817261716888-compute@developer.gserviceaccount.com`。该账号已具备访问 Secret Manager、Firestore 以及 Pub/Sub 的所需权限，因此无需在 Terraform 中再创建单独的服务账号。
