...
| 服务账号                | `google_service_account.ns_functions_sa`             | (逻辑名 `sa-ns-functions`)                                    | Y            | 专为 Cloud Functions 创建的服务账号。Terraform 中通过变量 (如 `var.sa_ns_functions_email`) 引用已创建或由Terraform创建的实际邮箱地址。参考: `infra/gcp/iam.tf`。 |
| IAM 绑定                | `google_project_iam_member.functions_sa_secret_accessor` (多个) | –                                                          | Y            | 将必要的IAM角色绑定到服务账号。参考: `infra/gcp/iam.tf`。                       |
